...
export const FilmsList = () => {
  ...
  const [filmLocal, setfilmLocal] = useState([]);
  ...
  const saveTolocal = (title) => {
    console.log(title); // вытщить из локала , преобразовать , добавить и обратно закинуть
    setfilmLocal(filmLocal.push(title));
    console.log(filmLocal);
      ...
      <ul className="list-group">
        {films.slice(0,15).map((film, index) => (
          <li className="list-group-item d-flex justify-content-between align-items-center" key={index}
            >
            {film.title} 
            <FilmItem title={film.title} filmLocal={filmLocal} saveTolocal={() => saveTolocal(film.title)}/>
          </li>
        ))}
      </ul>
    )
